...
async function getBestMove(fen) {
  ...
  const { data } = await axios.get(process.env.STOCKFISH_API, {
    params: { fen },
  });
...
async function getCurrentFen(page) {
  let fen = null;
  try {
...
async function playWithBestMove(page, callback) {
  if (busy) return;
  busy = true;
  ...
  try {
    ...
    const current_fen = await getCurrentFen(page);
...
async function clickon(page, el, timeout = 10000) {
...
async function playWithHuman(page) {
  ...
  playIntervalId = setInterval(async () => {
    await playWithBestMove(page, async () => {
      // start the new game
      await delay(5000);
      busy = false;
